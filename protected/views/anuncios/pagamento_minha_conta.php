...
Yii::app()->getClientScript()->registerScriptFile(Yii::app()->baseUrl . '/js/anunciar_pagamento_minha_conta.js?'.microtime(), CClientScript::POS_END);
...
<section class="header-row">
    <div class="row-line">
        <div class="container">
            <span class="row-title">Pagamentos</span>
            <div id="pagamento-box">
                <span class="pagamento-summary">
                        Total a pagar:
                        <span style="font-weight:bolder;" id="valor_total">
                                <?php
                                    if($somaOrdens == null) {
                                        echo 'R$ 0,00';
                                    }
                ...
                <?php
                    if($flgOrdensApagar && $somaOrdens != null) {
                        echo '<a class="link-pagamento bt-pay" id="btn-pagamento" onclick="_gaq.push([\'_trackEvent\', \'Pagamento\', \'click\', \'EFETUAR-PAGAMENTOS\']);">EFETUAR PAGAMENTO</a>';
                    }
                ?>
            </div>
        </div>
    ...
    <!-- confirmar boleto -->
<div class="lbox-ag" id="lbox-confirmacao-boleto" style="width: 580px !important;">
    <div class="texts-lbox-ag">
        <input type="button" id="close" class="fechar-form close" value="X">
        <div>
            <span id="texto_lightbox" class="ev-titleb" style="width: 530px !important;">Prosseguir com o pagamento?</br></span>
        </div>
    </div>
...
<!-- modal erro -->
<div class="lbox-ag" id="lbox-erro" style="width: 580px !important;">
    <div class="texts-lbox-ag">
        <input type="button" id="close" class="fechar-form close" value="X">
        <div>
            <span id="texto_erro_modal" class="ev-titleb" style="width: 530px !important;"></span>
        </div>
    </div>    
</div>
</section>
...
<section id="minha-conta-tabela">
    ...
    <div class="header-grid-pagamentos">
            <div class="cell-grid-pagamentos-bt"></div>
            <div class="cell-grid-pagamentos-sm">DATA</div>
            <div class="cell-grid-pagamentos-lg">DESCRIÇÃO</div>
            <div class="cell-grid-pagamentos-sm">VALOR</div>
            <div class="cell-grid-pagamentos-sm">STATUS</div>
            <div class="cell-grid-pagamentos-bt"></div>
    </div>
    ...
    <?php
        if($ordens != null) {
            ...
            foreach($ordens as $index => $ordem) {
                ...
                if(isset($ordem->transacoes->detalhes)) {
                  //$azpay_response = json_decode($ordem->transacoes->detalhes);
                  /*if (isset($azpay_response->operationId) && $azpay_response->operationId == '8')
                      $link_boleto = ' / <a href="'.$azpay_response->processor->Boleto->details->urlBoleto.'" target="_blank">Ver Boleto</a>';*/

                }

                $html = '<div class="row-grid-pagamentos">';
                if($ordem->status == 1 && !Transacoes::segundaViaBoleto($ordem)) {
                    $html .= '<div class="cell-grid-pagamentos-bt"><div class="radio-box-cadastra" style="margin: 0px -10px !important;"><input data-valor="'.$ordem->valor.'" class="selecao_ordem" type="checkbox" value="'.$ordem->id.'" style="display:none;"/><span class="info-label-radio"><i class="label-icon-radio"></i></span></div></div>';    
                }

                else {

                    $html .= '<div class="cell-grid-pagamentos-bt"><div class="radio-box-cadastra" style="margin: 0px -10px !important;"><input data-valor="'.$ordem->valor.'" class="selecao_ordem" type="checkbox" value="'.$ordem->id.'" style="display:none;"/></span></div></div>';      
                }
                
                $html .= '<div class="cell-grid-pagamentos-sm">'.Utils::formatDateTimeToView($ordem->data_criacao).'</div>';
                $html .= '<div class="cell-grid-pagamentos-lg">'.$ordem->descricao.'</div>';
                $html .= '<div class="cell-grid-pagamentos-sm">R$ '.Utils::formataValorView($ordem->valor).'</div>';
                ...
                if($ordem->status != Anuncio::$_status_ordem['PAGA']) {
                    ...
                    // se ja gerou boleto e ñ venceu
                    if(Transacoes::segundaViaBoleto($ordem) == true) {
                        echo '<div class="cell-grid-pagamentos-sm pendente"><a class="verboleto" data-codigoitau="'.$ordem->transacoes->codigo_itau.'" href="#">Ver boleto</a></div>';
                    }
                    else {
                        echo '<div class="cell-grid-pagamentos-sm pendente">' . Ordens::getTextoStatus($ordem->status) . $link_boleto . '</div>';    
                    }
                } else {

                    echo '<div class="cell-grid-pagamentos-sm pago">' . Ordens::getTextoStatus($ordem->status) . $link_boleto . '</div>';
                }


                echo    '<div class="cell-grid-pagamentos-bt">';
                            if($ordem->status != Anuncio::$_status_ordem['PAGA']) {
                                if(Transacoes::segundaViaBoleto($ordem) == false) {
                                    echo '<a class="cancelar-ordem delete-icon" id="'.$ordem->id.'" alt="excluir" a href="#"></a>';    
                                }
...
<div class="container payment_container" style="display:none;" id="novo-pagamento">

        <div class="pure-g">
        <div id="erro_pagto" style="margin-left:30px;font-size: 14px;font-weight: bolder;color: red;display:none;">Ocorreram os seguintes erros:<br/></div>
            <div class="payment-tabs">

                <div class="pure-g">

                    <div class="pure-u-1-2 form-boleto">
                        <div class="container">
                            <div class="box-pag-boleto">
                                <div class="quad-box-pag-boleto">
                                    <div class="title-pag-5">Clique para gerar o boleto</div>
                                    <br/>
                    ...
                    <div class="pure-u-1-2 form-card">
                        <?php $this->renderPartial('_anuncio_pagamento_cartao'); ?>
                    </div>
        ...
        <div class="linha-pagamento-5">
                ...
                echo CHtml::button("FINALIZAR", array("id"=>"submit-card","class"=>"botao-pag-3 btn btn-primary"));
                ?>
