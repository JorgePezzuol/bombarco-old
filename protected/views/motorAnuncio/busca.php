<?php
    Yii::app()->getClientScript()->registerScriptFile(Yii::app()->baseUrl . '/js/numeral.min.js', CClientScript::POS_END);
            ...
            Yii::app()->getClientScript()->registerScriptFile(Yii::app()->baseUrl . '/js/motor_busca.js?'.microtime(), CClientScript::POS_END);

?>
<section class="content">
    <?php
        $this->renderPartial('_filtro_busca', array('array_view'=>$array_view, 'array_view'=>$array_view));
    ?>

    <div class="line-white-3-listagem">
        <div class="container">

            <?php if ($array_view['total'] > 0): ?>

            <div class="checkbox-lis-emb">


                <?php if($array_view['busca'] != ""): ?>

                    <div class="resultados-box">

                        <span class="resultados-title"> 
                            Você buscou por "<?php echo $array_view["busca"];?>", foram encontrados <?php echo $array_view["total"];?> resultados 
                        </span>
                    </div>
                <?php endif;?>
                ...
                <form>
                    <div class="checkbox1">
                        <input type="radio" id="cb-list" name="teste" class="checkcad1" value="ASC" <?php if(isset($array_view['ordem']) && $array_view['ordem'] == "ASC") echo 'checked'; ?> >
                    </div>
                    <div class="checkbox2">
                        <input type="radio" id="cb-list2" name="teste" class="checkcad1" value="DESC" <?php if(isset($array_view['ordem']) && $array_view['ordem'] == "DESC") echo 'checked'; ?> >
                    </div>
                </form>
            </div>
            <?php endif ?>
            ...
            <div class= "box-tabela-bb">
                ...
                <?php
                    ...
                    foreach($array_view['motores'] as $motor) {
                        ...
                        echo '<li class="category-tabela">';
                            ...
                            echo MotorAnuncio::getThumb($motor, array('class'=>'bg-img-tabela'), true);                           

                            // se tiver destaque poe a classe com o selo de destaque
                            /*if(Embarcacoes::checkTurbo($motor, 'destaque_busca') == true) {
                                echo '<i class="faixa-destaque-emba"></i>';
                            }*/
                            ...
                            echo '<div class="textos-tabela-bb">';
                                ...
                                echo '<h2 class="text-tabela-bb-title">'.$motor->motorFabricantes->titulo . ' ' . $motor->motorTipos->titulo. ' '. '<b>'.$titulo.'</b></h2>';
                                

                                //if ($motor->status == Embarcacoes::ACTIVE) {

                                    echo '<h2 class="text-tabela-bb-ano">Potência: <b>'.$motor->potencia.'</b></h2>';
                                    echo '<h2 class="text-tabela-bb-estado">Estado: <b>' . Embarcacoes::$_estado[$motor->estado] . '</b></h2>';
                                    echo '<h2 class="text-tabela-bb-price">';
                                    echo ($motor->valor > 0?"R$ " . number_format($motor->valor, 2, ',', '.'):"R$ não informado");
                                    echo '</h2>';

                                    echo '<div  style="cursor:pointer;" class="balao_contato" data-email="@email" data-embarcid="'. $motor->id . '" data-link="'.MotorAnuncio::gerarLinkAbsoluto($motor).'" data-valor="'.$motor->valor.'" data-titulo="'.MotorAnuncio::nomeAnuncio($motor).'">';
                                    //echo '<img style="cursor:pointer; float:left;" class="balao" data-email="@email" data-embarcid="'. $motor->id . '" src="'. Yii::app()->createUrl("img/icon_chat.png"). '"/>';
                ...
                <?php if (count($array_view['motores']) == 0): ?>
                ...
                <?php if (count($array_view['motores']) == Embarcacoes::LIMIT_SEARCH): ?>
                    ...
                    <div class="div-btn-carregar-list ax-track-event" data-ax-trackname="btn ver mais listagem">
                        <a class="botao-carregar-list" id="carregar-list"
                             ...
                             data-estado="<?php echo $array_view['estado']; ?>"
                             data-marca="<?php echo $array_view['marca']; ?>"
                             data-preco-min="<?php echo $array_view['preco-min']; ?>"
                             data-preco-max="<?php echo $array_view['preco-max']; ?>"
                             data-tipo="<?php echo $array_view['tipo']; ?>"
                             data-ordem="<?php echo $array_view['ordem']; ?>"
                             data-busca="<?php echo $array_view['busca']; ?>">VER MAIS</a>
                    </div>
    ...
    <?php /*$cpm = Embarcacoes::cpm($array_view['macro']); ?>
