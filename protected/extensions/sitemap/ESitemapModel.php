	...
	public function getPathFormat()
	{
		if ( $this->_pathFormat === null )
		{
			...
			foreach( $this->params as $key=>$attribute )
			{
				...
				$route .= '/' . $this->formatParam( $attribute );
			}
			//$this->_pathFormat = Yii::app()->createAbsoluteUrl($this->route, $routeParams);
			$this->_pathFormat = Yii::app()->createAbsoluteUrl($route);

			if (strpos($this->_pathFormat, "http://www") === FALSE) {				
				$tmp = explode("http://", $this->_pathFormat);
				$this->_pathFormat = "http://www." . $tmp[1];
			}
