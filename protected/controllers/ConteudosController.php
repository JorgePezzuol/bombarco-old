...
class ConteudosController extends GxController {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow',
                ...
                'expression'=> function() {
                    if(Yii::app()->user->isConteudo() || Yii::app()->user->isAdmin() || Yii::app()->user->isAtendimento() || Yii::app()->user->isSeo()) {
                        return true;
                    }
                    return false;
                }
    ...
    public function actionCreate() {
        ...
        Yii::app()->getClientScript()->registerCssFile(Yii::app()->theme->baseUrl . '/css/jquery-ui.css?'.microtime());
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->theme->baseUrl . '/js/jquery-ui.js?'.microtime());
    ...
    public function actionUpdate($id) {
        ...
        // scripts
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->baseUrl . '/js/jquery-mask.js', CClientScript::POS_END);
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->baseUrl . '/js/admin_conteudos.js?e=ggdfgfdg', CClientScript::POS_END);
    ...
    public function actionUpdateFoto() {
        ...
        if (isset($_POST)) {
            ...
            $conteudoImagem = ConteudoImagens::model()->findByPk((int) $_GET['conteudo_imagem_id']);

            if ($conteudoImagem == null) {
                $conteudoImagem = new ConteudoImagens;
                $conteudoImagem->conteudos_id = $_GET['conteudos_id'];
            }

            //$conteudoImagem->imagem = CUploadedFile::getInstance($conteudoImagem, 'imagem');
            // $imagem = CUploadedFile::getInstanceByName('ConteudoImagens[imagem]');
            //$conteudoImagem->imagem = CUploadedFile::getInstance($conteudoImagem, 'ConteudoImagens[imagem]');            

            $conteudoImagem->imagem = CUploadedFile::getInstanceByName('ConteudoImagens[imagem]');  
    ...
    public function actionNoticias() {
        ...
        if ($ajax != null && $ajax == true) {
            ...
            $breadcrumbs[] = array('texto' => 'Lançamento de Lanchas', 'link' => Yii::app()->createUrl('comunidade/noticias'));
    ...
    public function actionNoticiaDetalhe($slug) {
        ...
        if($noticia == null) {
            $this->redirect(Yii::app()->createUrl("comunidade/noticias"));
        }
    ...
    public function actionPrimeiroBarcoDetalhe($slug) {
        ...
        if ($primeiro_barco == null) {
            $this->redirect(Yii::app()->createUrl("comunidade/primeiro-barco"));
        }
    ...
    public function actionTesteBombarco() {
        ...
        // Title
        $this->setPageTitle('Raio X Bombarco');
        ...
        $criteria = new CDbCriteria();
        $criteria->with = array('conteudoImagens', 'conteudoCategorias', 'embarcacaoMacros');
        $criteria->condition = 't.macro = "T" AND t.status = 1';
        $criteria->order = 'data DESC';
        $criteria->limit = Conteudos::LIMIT_SEARCH;
        ...
        if ($ajax != null && $ajax == true) {
            ...
            foreach ($teste_bombarco as $key => $value) {
                ...
                $youtubeID = Utils::getYoutubeID($value->video);

                $html .= '<li class="category-videos–lw4">';
                $html .= '<input class="video-url" type="hidden" value="//www.youtube-nocookie.com/embed/'.$youtubeID.'?autoplay=1&enablejsapi=1&version=3&hd=1&playerapiid=ytplayer&autohide=1&showinfo=0" />';
            ...
            $breadcrumbs[] = array('texto' => 'Home', 'link' => Yii::app()->homeUrl);
            $breadcrumbs[] = array('texto' => 'Comunidade', 'link' => Yii::app()->createUrl('comunidade'));
            $breadcrumbs[] = array('texto' => 'Teste de lanchas', 'link' => Yii::app()->createUrl('comunidade/raio-x'));

            // Últimos 3 videos
            $ultimos_videos = Conteudos::model()->with('conteudoImagens')->findAllByAttributes(array('macro' => 'T', 'status' => 1), array('order' => 't.data DESC, t.id DESC', 'limit' => 3));
    ...
    public function actionBlogDetalhe($slug) {
        ...
        if($blog == null) {
            $this->redirect(Yii::app()->createUrl("comunidade/blog"));
        }
