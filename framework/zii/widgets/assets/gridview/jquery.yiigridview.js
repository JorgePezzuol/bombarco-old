				...
				if (settings.ajaxUpdate.length > 0) {
					$(document).on('click.yiiGridView', settings.updateSelector, function () {
						...
						if (settings.enableHistory && window.History.enabled) {
							...
							window.History.pushState(null, document.title, decodeURIComponent(jQuery.param.querystring(url[0], params)));
						} else {
				...
				$(document).on('change.yiiGridView keydown.yiiGridView', settings.filterSelector, function (event) {
					...
					if (settings.enableHistory && settings.ajaxUpdate !== false && window.History.enabled) {
						...
						var url = $('#' + id).yiiGridView('getUrl'),
							params = $.deparam.querystring(jQuery.param.querystring(url, data));

						delete params[settings.ajaxVar];
						window.History.pushState(null, document.title, decodeURIComponent(jQuery.param.querystring(url.substr(0, url.indexOf('?')), params)));
					} else {
		...
		update: function (options) {
			...
			return this.each(function () {
				...
				if (settings.ajaxUpdate !== false) {
					if(settings.ajaxVar) {
						if(options.url.indexOf('?') > 0){
							options.url += "&" + settings.ajaxVar + '=' + id;
						}else{
							options.url += "?" + settings.ajaxVar + '=' + id;
						}
					}
