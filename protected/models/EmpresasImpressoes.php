...
Yii::import('application.models._base.BaseEmpresasImpressoes');

class EmpresasImpressoes extends BaseEmpresasImpressoes
{
	...
	public static function addViews($model) {
		...
		if ($model != null && count($model) > 0) {
			...
			self::model()->updateCounters(array('views' => 1), 'limitdate > NOW() AND limitviews > views AND empresas_id IN ( ' . implode(',',$ids) . ' )');	
